...
$activityExpr = "CASE WHEN (u.last_activity > NOW() - INTERVAL 5 MINUTE OR u.lastlogin > NOW() - INTERVAL 5 MINUTE) THEN 1 ELSE 0 END";
...
if ($filterActivity === 'active') {
    $whereClauses[] = "(" . $activityExpr . ") = 1";
} elseif ($filterActivity === 'inactive') {
    $whereClauses[] = "(" . $activityExpr . ") = 0";
}
...
if ($squadTableExists) {
    $sql = "SELECT u.userID, u.username, u.lastlogin, u.email, u.registerdate, " . $activityExpr . " AS activity, s.squadID 
            FROM users u
            LEFT JOIN plugins_squads_members s ON u.userID = s.userID";
} else {
    $sql = "SELECT u.userID, u.username, u.lastlogin, u.email, u.registerdate, " . $activityExpr . " AS activity, NULL as squadID
            FROM users u";
}
...
<div class="card shadow-sm mt-4">
    <div class="card-header">
        <div class="card-title">
            <i class="bi bi-box-arrow-in-right"></i> <?= $languageService->get('lastlogin_activity_control') ?>
        </div>
    </div>
    <div class="card-body">
        ...
        <div class="table-responsive">
            <table id="userlistTable" class="table" style="width:100%">
                <thead>
                    <tr>
                ...
                <tbody>
                    ...
                    while ($item = $result->fetch_assoc()):
                        ...
                        <tr>
                            <td><?= $number++ ?></td>
                            <td><a href="admincenter.php?site=user_roles&action=edit_user&userID=<?= (int)$userID ?>" target="_blank"><?= $username ?></a></td>
                            <?php if ($squadTableExists): ?>
